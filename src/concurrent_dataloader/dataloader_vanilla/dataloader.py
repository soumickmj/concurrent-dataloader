...
from concurrent_dataloader.dataloader_vanilla.fetch import _IterableDatasetFetcher
from concurrent_dataloader.dataloader_vanilla.fetch import _MapDatasetFetcher
from concurrent_dataloader.dataloader_vanilla.worker import get_worker_info
...
class DataLoader(Generic[T_co]):
    ...
    def __iter__(self) -> "_BaseDataLoaderIter":
        if not self.persistent_workers or self.num_workers <= 0:
        ...
        def _create_warning_msg(num_worker_suggest, num_worker_created, cpuset_checked):
            ...
            suggested_max_worker_msg = (
                (
                    ...
                    ).format(num_worker_suggest, ("" if cpuset_checked else " (`cpuset` is not taken into account)"))
                )
...
class _BaseDataLoaderIter(object):
    def __init__(self, loader: DataLoader) -> None:
        ...
        self._num_yielded = 0
        self._profile_name = (
            f"enumerate(DataLoader)#{self.__class__.__name__}.__next__"
        )
    ...
    def _try_get_data(self, timeout=_utils.MP_STATUS_CHECK_INTERVAL):
        ...
        try:
            ...
            try:
                ...
                fds_limit_margin = 10
                fs = [tempfile.NamedTemporaryFile() for _ in range(fds_limit_margin)]
            except OSError as e:
