...
parser.add_argument(
    "-j", "--num-workers", default=4, type=int, metavar="N", help="number of data loading workers (default: 4)"
)
parser.add_argument("--epochs", default=10, type=int, metavar="N", help="number of total epochs to run")  # default 90
...
parser.add_argument("--fetch-impl", type=str, default="asyncio", help="threaded | asyncio")
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        logging.getLogger("timeline").debug(
            json.dumps({"item": "training_batch_to_device", "id": batch_timeline_id, "end_time": time.time()})
        )

        # compute output
        training_batch_timeline_id = abs(hash(time.time() + epoch))
        logging.getLogger("timeline").debug(
            json.dumps({"item": "run_training_batch", "id": training_batch_timeline_id, "start_time": time.time()})
        )
        output = model(images)
        ...
        optimizer.step()
        logging.getLogger("timeline").debug(
            json.dumps({"item": "run_training_batch", "id": training_batch_timeline_id, "end_time": time.time()})
        )
